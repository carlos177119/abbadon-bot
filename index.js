...
const config = {
    XBOT: '❉AmpibI❉', 
    instagram: 'https://www.instagram.com/ssanchez.pty/', 
    nomer: 'wa.me/50769258167',
...
client.on('group-participants-update', async (anu) => {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '❬❗❭ espera, el proceso',
				success: '️❬ ✔ ❭ éxito 🖤',
				error: {
					stick: 'Bien fallido ;(, intenta repetir ^_^',
					Iv: 'lo siento enlace inválido☹️'
				},
				only: {
					group: '❬❗❭ solo grupo ',
					ownerG: '❬❗❭ solo propietario ',
					ownerB: '❬❗❭ solo propietario ',
					admin: '❬❗❭ solo administrador ',
					Badmin: '❬❗❭ bot debe ser administrador '
				}
			}
          ...
          case 'ping':
            ...
            case 'kangcopet':
				...
				case 'info':
					...
					teks = `*Nombre bot* : ${me.name}\n*DUEÑO* : *Abbadon*\n*AUTOR* : Carlito\n*Numero de Bot* : @${me.jid.split('@')[0]}\n*Prefijo* : ${prefix}\n*Contactos bloquiados* : ${blocked.length}\n*Bot activo el* : ${kyun(uptime)}`
					buffer = await getBuffer(me.imgUrl)
					client.sendMessage(from, buffer, image, {caption: teks, contextInfo:{mentionedJid: [me.jid]}})
					break
				case 'blocklist': 
					teks =  LISTA DE BLOQUEO :\n'
				...
				case 'ocr': 
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply('enviar foto con pie de foto ${prefix}𝗼𝗰𝗿')
					}
					break
				case 'stiker': 
				case 'sticker':
				case 's':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply(`Enviar fotos con subtítulos ${prefix}sticker atau reply/tag gambar`)
					}
					break
				case 'getses':
            ...
            break	
				...
				case 'tts':
					...
					rano = getRandom('.ogg')
					dtt.length > 300
					? reply('El texto significa Bambang😤')
               ...
               if (!isGroup) return reply(mess.only.group)
				...
				case 'limpiar':
				 ...
				 client.chatRead (from)
					...
					client.sendMessage(from, `Coje tu bloqueo perra ${body.slice(7)}@c.us`, text)
					break
                    case 'desbloquiar':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(mess.only.ownerB)
				    client.blockUser (`${body.slice(9)}@c.us`, "remove")
					client.sendMessage(from, `te desbloquiare por gay ${body.slice(9)}@c.us`, text)
				break
				case 'salir': 
				if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'bc': 
			   	...
			   	case 'setpp': 
                       ...
                       media = await client.downloadAndSaveMediaMessage(mek)
                        ...
                        reply('cambió correctamente el icono del grupo')
                break						
				case 'add':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					...
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args[0] === 'buka') {
					    reply(`¿tener éxito? abre el grupo pues`)
						client.groupSettingChange(from, GroupSettingChange.messageSend, false)
					} else if (args[0] === 'tutup') {
						reply(`¿tener éxito? abre el grupo zpues`)
            ...
            case 'owner':
            case 'creator':
       client.sendMessage(from, 'si pusistes este comando es porque eres gay y chupas nepe :)',MessageType.text, { quoted: mek} )
           break    
           case 'setname':
                if (!isGroup) return reply(mess.only.group)
			    if (!isGroupAdmins) return reply(mess.only.admin)
           ...
           case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`𝘆𝗮𝗵h @${mentioned[0].split('@')[0]} gay`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'promote':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Bienvenido, gay (+_+) :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					} else {
						mentions(`Bienvenido @${mentioned[0].split('@')[0]} gay (+_+)`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break	
			     	case 'kick':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Hasta la proxima gay :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
						mentions(`Hasta la proxima @${mentioned[0].split('@')[0]} gay`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
					if (!isGroup) return reply(mess.only.group)
					teks = `lista de los admin gay *${groupMetadata.subject}*\ntotales : ${groupAdmins.length}\n\n`
					no = 0
				...
				case 'welcome':
					...
					if (args.length < 1) return reply('Boo :𝘃')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('Ohhhhhhh !!!')
						welkom.push(from)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('❬ 𝗦𝗨𝗞𝗦𝗘𝗦 ❭ se unio un gay/se salio un gay')
					} else if (Number(args[0]) === 0) {
				...
				case 'clonar':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(' *A quien quieres clonar* ?') 
